import { DispatchError, ModuleError, PalletErrorMetadataLatest, PalletEventMetadataLatest, PalletStorageEntryMetadataLatest, PalletTxMetadataLatest, StorageKey } from '@dedot/codecs';
...
export type GenericStorageQuery<Rv extends RpcVersion = RpcVersion, T extends AnyFunc = AnyFunc, KeyTypeOut extends any = undefined> = StorageQueryMethod<T> & {
    meta: PalletStorageEntryMetadataLatest;
    rawKey: (...args: Parameters<T>) => StorageKey;
} & (KeyTypeOut extends undefined ? {} : Rv extends RpcLegacy ? {
...
export interface PalletEvent<Pallet extends string = string, EventName extends string = string, Data extends any = any> {
    pallet: Pallet;
    palletEvent: Data extends undefined ? EventName : Data extends null ? {
        name: EventName;
    } : {
        name: EventName;
        data: Data;
...
export interface GenericPalletEvent<_ extends RpcVersion = RpcVersion, Pallet extends string = string, EventName extends string = string, Data extends any = any> {
...
export type GenericChainEvents<Rv extends RpcVersion = RpcVersion, Pallet extends string = string, EventName extends string = string, Data extends any = any> = Record<Pallet, Record<EventName, GenericPalletEvent<Rv, Pallet, EventName, Data>>>;
