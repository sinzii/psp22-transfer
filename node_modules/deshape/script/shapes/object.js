"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.object = exports.optionalField = exports.field = void 0;
const mod_js_1 = require("../common/mod.js");
const option_js_1 = require("./option.js");
function field(key, $value) {
    return (0, mod_js_1.createShape)({
        metadata: (0, mod_js_1.metadata)("$.field", field, key, $value),
        staticSize: $value.staticSize,
        subEncode(buffer, value) {
            $value.subEncode(buffer, value[key]);
        },
        subDecode(buffer) {
            return { [key]: $value.subDecode(buffer) };
        },
        subAssert(assert) {
            $value.subAssert(assert.key(this, key));
        },
    });
}
exports.field = field;
function optionalField(key, $value) {
    const $option = (0, option_js_1.option)($value);
    return (0, mod_js_1.createShape)({
        metadata: (0, mod_js_1.metadata)("$.optionalField", optionalField, key, $value),
        staticSize: $value.staticSize,
        subEncode(buffer, value) {
            $option.subEncode(buffer, value[key]);
        },
        subDecode(buffer) {
            if (buffer.array[buffer.index++]) {
                return { [key]: $value.subDecode(buffer) };
            }
            else {
                return {};
            }
        },
        subAssert(assert) {
            assert.typeof(this, "object");
            assert.nonNull(this);
            if (key in assert.value) {
                $option.subAssert(assert.key(this, key));
            }
        },
    });
}
exports.optionalField = optionalField;
function object(...members) {
    return (0, mod_js_1.createShape)({
        metadata: (0, mod_js_1.metadata)("$.object", (object), ...members),
        staticSize: members.map((x) => x.staticSize).reduce((a, b) => a + b, 0),
        subEncode: generateEncode(members),
        subDecode: generateDecode(members),
        subAssert(assert) {
            assert.typeof(this, "object");
            assert.nonNull(this);
            for (const member of members) {
                member.subAssert(assert);
            }
        },
    });
}
exports.object = object;
function generateEncode(members) {
    return (buffer, value) => {
        members.forEach(member => {
            member.subEncode(buffer, value);
        });
    };
}
function generateDecode(members) {
    return (buffer) => {
        return members.reduce((o, member) => {
            return { ...o, ...member.subDecode(buffer) };
        }, {});
    };
}
//# sourceMappingURL=object.js.map